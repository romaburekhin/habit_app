...
beforeEach(() => {
  ...
  createHabit(db, userId, 'Exercise', 30) // id=1
  createHabit(db, userId, 'Read', 20)     // id=2
})

describe('listCompletions', () => {
  it('returns empty array when no completions', () => {
    expect(listCompletions(db, userId)).toEqual([])
  })
  ...
  it('filters by date range', () => {
    ...
    const results = listCompletions(db, userId, '2026-03-07', '2026-03-10')
  ...
  it('returns all completions when no range given', () => {
    ...
    expect(listCompletions(db, userId)).toHaveLength(2)
  })
})
...
describe('toggleCompletion', () => {
  ...
  it('deletes existing completion and returns created=false', () => {
    ...
    expect(listCompletions(db, userId)).toHaveLength(0)
  })
